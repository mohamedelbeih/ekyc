...
if page_name in st.session_state['selected_verifications']:
        ...
        with st.container():
            ...
            with col2:
                if st.checkbox("Personal_image_"+face.split("_")[-1],value=True):
                    os.system("cp verify/Input/"+face + " verify/selected/"+face)
                else:
                    os.system("rm verify/selected/"+face)
    ...
    if captured_picture or uploaded_picture:
        ...
        with st.spinner('Please wait .. Detecting faces in image'):
            selected_faces = os.listdir("verify/selected/")
            selected_faces = [ "verify/selected/" + x for x in selected_faces]
